...
export default function LiveCamera() {
  ...
  return (
    <div className="min-h-screen bg-[#111111] text-neutral-200 font-sans flex h-screen overflow-hidden">
      ...
      <aside className="w-80 bg-[#141414] border-l border-neutral-800 flex flex-col h-full hidden xl:flex shadow-2xl">
        <div className="p-5 border-b border-neutral-800 flex items-center gap-3">
          <span className="text-lime-400 text-lg">🔔</span>
          <h2 className="font-bold text-white tracking-wide">Real-time Alerts</h2>
        </div>

        <div className="flex-1 overflow-y-auto p-4 space-y-3 scrollbar-thin scrollbar-thumb-neutral-700">
          {alerts.map((alert) => (
            <div key={alert.id} className="bg-[#1a1a1a] border border-neutral-700 hover:border-neutral-500 transition-colors rounded-lg p-4 flex flex-col gap-1 cursor-pointer">
              <div className="flex justify-between items-start">
                <span className={`text-[10px] font-bold tracking-wider ${alert.titleColor}`}>{alert.type}</span>
                <span className="text-[10px] text-neutral-500">{alert.time}</span>
              </div>
              <h3 className="text-lg font-bold text-white mt-1">{alert.plate}</h3>
              <p className="text-xs text-neutral-400 mb-2">{alert.desc}</p>
              
              <div className="flex justify-between items-center mt-1">
                <span className="text-[9px] font-bold tracking-widest text-lime-500 border border-lime-500/30 bg-lime-500/10 px-2 py-1 rounded">
                  {alert.priority}
                </span>
        ...
        <div className="p-4 border-t border-neutral-800 text-center">
          <Link href="/Analytics" className="text-xs text-lime-500 hover:text-lime-400 font-bold transition-colors flex items-center justify-center gap-2">
            View All Alert History <span>&gt;</span>
          </Link>
        </div>
      </aside>
      </div>
